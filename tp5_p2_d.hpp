...
void agregar_pila(tpila &p,int nuevo)
{
	if(pila_llena(p)!=true)
	{
		...
		else
		{
			if(p.c1[m-1]<m*2)
			{
				p.c3[p.c1[m-1]-m]=nuevo;
			}
			else
			{
				p.c3[p.c1[m-1]-m*2]=nuevo;
			}
		}
	}
}
...
int quitar_pila(tpila &p)
{
	...
	if(pila_vacia(p)!=true)
	{
		if(p.c1[m-1]>m*2)
		{
			extraido=p.c1[p.c1[m-1]];
			p.c1[m-1]= p.c1[m-1] - 1;
			cout<<"RESTAR: "<<p.c1[m-1]<<endl;
		}
		else
		{
			if(p.c1[m-1]>m)
			{
				extraido=p.c2[p.c1[m-1]];
				p.c1[m-1]=p.c1[m-1]-1;
				cout<<"RESTAR: "<<p.c1[m-1]<<endl;
...
int tope_pila(tpila p)
{
	...
	if(pila_vacia(p)!=true)
	{
		...
		else
		{
			if(p.c1[m-1]<m*2-1)
			{
				retornado=p.c2[p.c1[m-1]];
			}
			else
			{
				retornado=p.c1[p.c1[m-1]];
			}
		}
	}
